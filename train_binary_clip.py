...
# from vgg import vgg16_bn as AlexNet
...
def train(model, loss_fn, optimizer, param, loader_train, loader_test, loader_val=None):
    ...
    for epoch in range(param['num_epochs']):
        ...
        val1(model, loader_test)

# train
def train_pp(model, loss_fn, optimizer, param, loader_train, loader_test, bin_op, k=40, l=5,loader_val=None):
    val1(model, loader_test)
    model.train()
    best_accu = 0
    count_lr_m = 0
    for epoch in range(param['num_epochs']):
        ...
        if epoch <= 150:
            tmp_accu = val1(model, loader_test)
        else:
            tmp_accu = val2(model, loader_test, bin_op)
        ...
        if (epoch+1)%k==0 :
            ...
            lr = param['learning_rate'] * (0.5 ** count_lr_m)
